...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --num-skills) NUM_SKILLS="$2"; shift 2 ;;
    --)           shift; break;;
...
# ==============================================================================
# [NEW] WandB API Key 자동 로드 및 로그
# ==============================================================================
# 현재 스크립트 위치 기준으로 ../wandb_info/wandb_api_key 파일을 찾습니다.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
KEY_FILE="${SCRIPT_DIR}/../wandb_info/wandb_api_key"

if [[ -f "$KEY_FILE" ]]; then
  echo "[INFO] Loading WandB API key from $KEY_FILE"
  # 파일 내용을 읽어서 공백 제거 후 변수에 저장
  WANDB_KEY=$(cat "$KEY_FILE" | tr -d '[:space:]')
  
  # 환경 변수로 설정 (가장 확실한 방법)
  export WANDB_API_KEY="$WANDB_KEY"
else
  echo "[WARN] WandB key file not found at: $KEY_FILE"
  echo "       Proceeding with existing login session (if any)."
fi
...
RUN_NAME="${EXPERIMENT}"
if [[ -n "$NUM_SKILLS" ]]; then
  RUN_NAME="${RUN_NAME}-k${NUM_SKILLS}"
fi
...
# DIAYN Num Skills override
if [[ -n "$NUM_SKILLS" ]]; then
  PY_ARGS+=("DIAYN.NUM_SKILLS=${NUM_SKILLS}")
fi
...
# 3) 실험 실행
# cd experiments # skill-experiments 폴더에서 바로 실행

env -u LD_LIBRARY_PATH -u XLA_FLAGS \
  python skill_ov2_experiments/ppo/main.py "${PY_ARGS[@]}" 2>&1 | filter_ptx
