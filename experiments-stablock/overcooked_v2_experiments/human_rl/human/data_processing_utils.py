...
from .compatability import (
    convert_action_to_overcooked_v2,
    convert_state_to_overcooked_v2,
)
...
def df_traj_to_python_joint_traj(
    ...
    layout_name_v2 = TRIAL_NAME_TO_LAYOUT_V2[layout_name]
...
def joint_state_trajectory_to_single(
    ...
    **kwargs,
    ...
    env_v2 = joint_traj_data["metadatas"]["env_v2"][0]
    layout_name_v2 = joint_traj_data["metadatas"]["layout_name_v2"][0]
    ...
    obs_jit = jax.jit(env_v2.get_obs)
    ...
    for agent_idx in player_indices_to_convert:
        ...
        for i in range(len(states)):
            ...
            if featurize_states:
                ...
                action = convert_action_to_overcooked_v2(action)
                action = jnp.array([action], dtype=jnp.int32)

                # we do not want to swap since we are interested in agent observation - action pairs matching
                # it is not important that the states are identical (i.e. we don't care if the agent order is swapped)
                convertedState = convert_state_to_overcooked_v2(
                    state, layout_name_v2, perform_swap=False
                )
                obs_all = obs_jit(convertedState)
                # obs_all = env_v2.get_obs(convertedState)
                obs = obs_all[f"agent_{agent_idx}"]
