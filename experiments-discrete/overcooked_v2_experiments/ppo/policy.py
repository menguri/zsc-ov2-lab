...
class PPOPolicy(AbstractPolicy):
    ...
    def compute_action(self, obs, done, hstate, key, params=None, obs_history=None, act_history=None, partner_prediction=None, **kwargs):
        if params is None:
            params = self.params
        ...
        # partner_prediction = None # Remove initialization to None to allow argument
        ...
        obs_hist_in = None
        act_hist_in = None
        # partner_prediction = None # Don't reset if passed
        ...
        # [E3D Support] If partner_prediction is passed from caller (eval/rollout.py), perform dimension check
        if partner_prediction is not None:
            # Ensure correct dimensions for network.apply
            # Expected shape for ActorCriticRNN: (Time, Batch, Dim)
            if partner_prediction.ndim == 1:
                # (Dim,) -> (1, 1, Dim)
                partner_prediction = partner_prediction[jnp.newaxis, jnp.newaxis, ...]
            elif partner_prediction.ndim == 2:
