...
class ActorCriticRNN(ActorCriticBase):
    ...
    def __call__(self, hidden, x, train=False, partner_prediction=None, obs_history=None, act_history=None):
        ...
        # [Log] Initial Input Shape (One-time debug via JIT compile time print)
        # Note: This prints during JIT compilation, so you see shapes, not actual values
        # We can't use jax.debug.print easily for shapes inside compact unless we use a flag or just print
        print(f"[ActorCriticRNN] RNN Input Embedding Shape: {embedding.shape}")
