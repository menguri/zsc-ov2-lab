...
# Add project root (experiments-discrete) to sys.path to ensure local package is found first
...
from overcooked_v2_experiments.eval.sensitivity import analyze_sensitivity_pairing
...
def visualize_ppo_policy(
    ...
    # Detect OV type
    env_name = config["env"].get("ENV_NAME", "")
    ov_type = "ov2"
    if "overcooked" in env_name and "v2" not in env_name:
        ov_type = "ov1"
    ...
    for first_level, first_level_runs in all_params.items():
        ...
        for second_level, pairing in first_level_runs.items():
            ...
            ego_direction_arg = False

            if "E3T" in current_algs:
                alg_arg = "E3T"
            elif "E3D" in current_algs:
                alg_arg = "E3D"
                # Extract ego_direction from valid E3D config
                # Find first E3D config
                for cfg in current_configs:
                     if cfg.get("ALG_NAME") == "E3D":
                         ego_direction_arg = cfg.get("EGO_DIRECTION", False)
                         break
            else:
                alg_arg = current_algs[0]
            
            if sensitivity_analysis and alg_arg == "E3D":
                 print(f"Running Sensitivity Analysis for {first_level}/{second_level}")
                 sensitivity_csv = run_base_dir / first_level / second_level / "sensitivity.csv"
                 # Ensure dir exists e.g. runs/run_0/ckpt_final/sensitivity.csv
                 os.makedirs(os.path.dirname(sensitivity_csv), exist_ok=True)
                 
                 policies = []
                 for i in range(num_actors):
                      # pairing is list/tuple of PPOParams
                      agent_params = pairing[i]
                      policies.append(PPOPolicy(agent_params.params, current_configs[i]))
                 policy_pairing_obj = PolicyPairing(*policies)
                 
                 env_kwargs_no_layout = copy.deepcopy(env_kwargs)
                 layout_name = env_kwargs_no_layout.pop("layout")
                 
                 analyze_sensitivity_pairing(
                     policy_pairing_obj,
                     layout_name,
                     key,
                     env_kwargs_no_layout,
                     str(sensitivity_csv),
                     ego_direction=ego_direction_arg
                 )
                 
                 # Dummy result for compatibility
                 from overcooked_v2_experiments.eval.evaluate import VizResult
                 viz_result = VizResult(0, 0, None, None, None) 
                 results_structure[first_level][second_level] = viz_result
                 continue

            # JIT Key
            jit_key = (current_algs, ego_direction_arg, ov_type) # Include ego_direction and ov_type in key
            ...
            if jit_key not in jit_cache:
                ...
                # Also capture ego_direction_arg
