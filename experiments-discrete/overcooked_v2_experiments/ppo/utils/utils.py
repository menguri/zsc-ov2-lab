...
def _infer_run_suffix(config) -> str:
    ...
    if alg_name == "E3T" or "e3t" in str(wandb_name).lower():
        suffix = f"e3t"
        # STL (Anchor) 활성화 시 suffix에 stl 추가
        if config.get("model", {}).get("anchor", False):
            suffix = f"stl"

    # E3D 알고리즘인 경우 suffix에 e3d-{true/false} 추가
    if alg_name == "E3D" or "e3d" in str(wandb_name).lower():
        # EGO_DIRECTION 값에 따라 접미사 결정
        ego_dir = config.get("EGO_DIRECTION", False)
        if str(ego_dir).lower() == "true":
            suffix = "e3d-true"
        else:
            suffix = "e3d-false"
