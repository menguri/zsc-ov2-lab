...
import overcooked_v2_experiments.ppo.ippo as ippo_module
print(f"[MAINDBG] ippo module file = {ippo_module.__file__}")
...
def single_run_with_viz(config):
    ...
    # E3D 알고리즘인 경우, run_name에도 ego_direction 정보(true/false)가 드러나도록 수정
    if config.get("ALG_NAME") == "E3D":
        ego_dir = str(config.get("EGO_DIRECTION", False)).lower()
        suffix_str = "e3d-true" if ego_dir == "true" else "e3d-false"
        
        # 이미 이름에 반영되어 있지 않다면 추가/수정
        if "e3d-true" not in run_name and "e3d-false" not in run_name:
            if "e3d" in run_name:
                run_name = run_name.replace("e3d", suffix_str)
            else:
                run_name = f"{run_name}-{suffix_str}"
