...
from .models.e3t import DiClusterEncoder

# Module load debug
print(f"[IPPO] Loaded from {__file__}", flush=True)
...
def _find_kernel_shapes(node, prefix=""):
    shapes = []
    if hasattr(node, "shape") and prefix.endswith("/kernel"):
        shapes.append((prefix, tuple(node.shape)))
        return shapes
    for key, value in _iter_items(node):
        new_prefix = f"{prefix}/{key}" if prefix else str(key)
        shapes.extend(_find_kernel_shapes(value, new_prefix))
    return shapes


def _get_gru_input_dim(params):
    kernel_shapes = _find_kernel_shapes(params)
    for path, shape in kernel_shapes:
        if path.endswith("/GRUCell_1/ir/kernel") or path.endswith("/GRUCell_0/ir/kernel"):
            return shape[0], path, shape
    if kernel_shapes:
        path, shape = kernel_shapes[0]
        return shape[0], path, shape
...
def make_train(
    ...
    # E3T 설정 로드
    alg_config = config.get("alg", {})
    alg_name = config.get("ALG_NAME", alg_config.get("ALG_NAME", "SP"))
    e3t_epsilon = config.get("E3T_EPSILON", alg_config.get("E3T_EPSILON", 0.05))
    e3d_epsilon = config.get("E3D_EPSILON", alg_config.get("E3D_EPSILON", 0.5))
    ego_direction = config.get("EGO_DIRECTION", alg_config.get("EGO_DIRECTION", False))
    use_partner_modeling = config.get("USE_PARTNER_MODELING", alg_config.get("USE_PARTNER_MODELING", True))
    ...
    # E3D/E3T env type detection from env_config
    # Usually "grounded_coord_simple" implies OV2, "cramped_room" is OV1 legacy or OV2.
    # We follow the convention in DiClusterEncoder or pass explicit env string.
    # config.env.ENV_NAME tells us.
    env_name_cfg = env_config.get("ENV_NAME", "")
    # Default detection
    ov_type = "ov2"
    if "overcooked" in env_name_cfg.lower() and "v2" not in env_name_cfg.lower():
        # Maybe OV1? But this repo focuses on OV2.
        # Check layout or other indicators if needed.
        ov_type = "ov1"
    ...
    def train(
        ...
        # E3T is fixed to 5, E3D can be configured
        history_len = 5
        if alg_name == "E3D":
             history_len = model_config.get("HISTORY_LENGTH", 5)
        ...
        if alg_name == "E3T" and use_partner_modeling:
             ...
             # Shape: (1, Batch, Context=HistoryLen, H, W, C)
             dummy_obs_hist = jnp.zeros((1, model_config["NUM_ENVS"], history_len, *env.observation_space().shape))
             dummy_act_hist = jnp.zeros((1, model_config["NUM_ENVS"], history_len), dtype=jnp.int32)

        # [E3D Dummy Init]
        if alg_name == "E3D" and use_partner_modeling:
             # E3D's partner prediction is the z_latent (16 or 32 dims)
             z_dim = 16
             if ego_direction:
                 z_dim = 32
             # Shape: (Time=1, Batch=NUM_ENVS, LatentDim=z_dim)
             dummy_partner_prediction = jnp.zeros((1, model_config["NUM_ENVS"], z_dim))
             # E3D doesn't use the internal predictor, pass None to avoid creating unused params
             dummy_obs_hist = None
             dummy_act_hist = None
        ...
        # Print actual GRU input dim (embedding+z) once at init
        gru_input_dim, gru_kernel_path, gru_kernel_shape = _get_gru_input_dim(network_params)
        print(
            f"[INIT] alg_name={alg_name} use_partner_modeling={use_partner_modeling} "
            f"ego_direction={ego_direction}",
            flush=True,
        )
        print(
            f"[INIT] GRU input dim={gru_input_dim}, kernel_shape={gru_kernel_shape}, "
            f"path={gru_kernel_path}",
            flush=True,
        )
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(env_step_state, unused):
                ...
                # 2. Partner Prediction (E3T / E3D)
                partner_prediction = None
                
                # [E3D Implementation]
                if alg_name == "E3D" and use_partner_modeling:
                    # (1) Compute Latents from History
                    # DiClusterEncoder: Rule-based, stateless (params={})
                    # obs_history: (Batch, 5, H, W, C)
                    # For t < 5 (start of episode), the history contains zeros or old steps masked by reset_mask.
                    # DiClusterEncoder handles valid frames within the window if implemented robustly, 
                    # but since we mask history with reset_mask, old zeros correspond to "no movement" or (0,0).
                    # DiClusterEncoder logic uses frame 0 and frame -1.
                    # If history is [0,0,0,0, Obs_t], then frame 0 is empty.
                    # However, we want "Initial Pos" to "Current Pos".
                    # If reset just happened, obs_history is [0,0,0,0, Obs_0]. 
                    # We might need to handle this.
                    # Simplified assumption: Zero frame yields (0,0) pos -> angle 0.
                    # User requested: "초반 길이5이전까지는 t=0부터 현재 t를 처음/마지막으로 설정하면 돼"
                    # But history buffer is fixed size.
                    # Instead of complicating DiClusterEncoder, we assume it extracts positions from first and last valid frames?
                    # Or simpler: Just call it. It takes obs_history[:, 0] and obs_history[:, -1].
                    
                    # Warning: DiClusterEncoder assumes obs shape to extract pos.
                    # We should rely on its internal logic or update it if needed.
                    # Here we treat it as a black box that takes (Batch, Time, ...) and returns z.
                    
                    z_ego_onehot, z_part_onehot = di_cluster_encoder.apply({}, obs_history, act_history, ov_type)
                    
                    # z_ego_onehot: (Batch, 16) - Ego movement
                    # z_part_onehot: (Batch, 16) - Partner movement
                    
                    # (2) Construct Interactions
                    # Ego(My) Input:
                    #  - If EGO_DIRECTION=True: [z_part_onehot, z_ego_onehot] (Target: Partner, Own)
                    #  - If EGO_DIRECTION=False: [z_part_onehot]
                    ...
                    # Partner(Opponent) Input (Self-play simulation):
                    # Partner sees ME (Ego) as their partner.
                    # So "z_other" for Partner is "z_ego".
                    # User requirement: "partner (partner perspective) z is random one-hot ... Partner's own z is properly calculated."
                    # Partner's own z (from their perspective) is their self-z (z_ego_onehot).
                    # Partner's other z (Ego) is Random.
                    
                    rng, rng_rand_z = jax.random.split(rng)
                    rand_idx = jax.random.randint(rng_rand_z, (model_config["NUM_ACTORS"],), 0, 16) # 16 clusters
                    rand_z_onehot = jax.nn.one_hot(rand_idx, 16)
                    
                    # Partner Input Construction:
                    #  - If EGO_DIRECTION=True: [z_other=Random, z_own=Real_Z_Ego]
                    #  - If EGO_DIRECTION=False: [z_other=Random]
                    
                    if ego_direction:
                        # Partner needs [Target(Random), Self(Real)]
                        # real_z_ego represents "Self" movement for each actor
                        fake_z_input = jnp.concatenate([rand_z_onehot, real_z_ego], axis=-1)
                    else:
                        fake_z_input = rand_z_onehot

                        
                    # (3) Select Input based on is_ego
                    # Ego uses Real Z, Partner uses Fake Z
                    # combined_z = jnp.where(is_ego[:, None], real_z_input, fake_z_input)

                    # [Modified] Partner also uses Real Z (Not Random)
                    combined_z = real_z_input
                    
                    # (Batch, Dim) -> (1, Batch, Dim)
                    partner_prediction = combined_prediction = combined_z[jnp.newaxis, ...]
                
                # [E3T Implementation]
                elif alg_name == "E3T" and use_partner_modeling:
                ...
                # Call network with partner_prediction (for E3T or E3D)
                # For E3D, obs_history=None ensures internal predictor is skipped
                obs_hist_arg = obs_history if alg_name == "E3T" else None
                hstate, pi, value = network.apply(train_state.params, hstate, ac_in, partner_prediction=partner_prediction, obs_history=obs_hist_arg, act_history=act_history)
                ...
                # [E3T/E3D] 파트너 행동 변경 로직 (Mixture Policy)
                ...
                if alg_name in ["E3T", "E3D"]:
                    ...
                    # 2. 베르누이 마스크 생성
                    current_epsilon = e3t_epsilon if alg_name == "E3T" else e3d_epsilon
                    mix_mask = jax.random.bernoulli(rng_mix, p=current_epsilon, shape=(model_config["NUM_ACTORS"],))
                    ...
                    action_for_env = jnp.where(mix_mask, rand_action, action_for_env)
            ...
            if alg_name == "E3D" and use_partner_modeling:
                z_ego_onehot, z_part_onehot = di_cluster_encoder.apply({}, obs_history, act_history, ov_type)
                if ego_direction:
                    real_z_input = jnp.concatenate([z_part_onehot, z_ego_onehot], axis=-1)
                else:
                    real_z_input = z_part_onehot
                partner_prediction = real_z_input[jnp.newaxis, ...]
            if alg_name == "E3T" and use_partner_modeling:
                # 1. Predict
                ...
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, targets):
                        ...
                        if alg_name == "E3D" and use_partner_modeling:
                            ...
                            z_ego_onehot, z_part_onehot = di_cluster_encoder.apply(
                                {}, obs_hist_flat, act_hist_flat, ov_type
                            )
                            if ego_direction:
                                real_z_input = jnp.concatenate(
                                    [z_part_onehot, z_ego_onehot], axis=-1
                                )
                            else:
                                real_z_input = z_part_onehot
                            partner_prediction = jax.lax.stop_gradient(real_z_input)
                            # (Minibatch, Dim) -> (Minibatch, 1, Dim)
                            partner_prediction = partner_prediction[:, jnp.newaxis, :]
        ...
        # E3T History Buffers
        initial_obs_history = jnp.zeros((model_config["NUM_ACTORS"], history_len, *env.observation_space().shape))
        initial_act_history = jnp.zeros((model_config["NUM_ACTORS"], history_len), dtype=jnp.int32)
