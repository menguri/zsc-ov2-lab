...
while [[ $# -gt 0 ]]; do
  ...
  --nsteps)     MODEL_NUM_STEPS_OVERRIDE="$2"; shift 2;;
    ...
    --e3d-epsilon) E3D_EPSILON="$2"; shift 2;;
    --ego-direction) EGO_DIRECTION="$2"; shift 2;;
    --history-len) HISTORY_LEN_OVERRIDE="$2"; shift 2;;
    --mem-frac)   XLA_PYTHON_CLIENT_MEM_FRACTION="$2"; shift 2;;
    --fcp-device) FCP_DEVICE="$2"; shift 2 ;;
    --)           shift; break;;
    *)            EXTRA_OVERRIDES+=("$1"); shift 1;;
  esac
done
...
# E3D epsilon override
if [[ -v E3D_EPSILON && -n "$E3D_EPSILON" ]]; then
  PY_ARGS+=("E3D_EPSILON=${E3D_EPSILON}")
fi

if [[ -v HISTORY_LEN_OVERRIDE && -n "$HISTORY_LEN_OVERRIDE" ]]; then
  PY_ARGS+=("model.HISTORY_LENGTH=${HISTORY_LEN_OVERRIDE}")
fi

# EGO_DIRECTION override
if [[ -v EGO_DIRECTION && -n "$EGO_DIRECTION" ]]; then
  PY_ARGS+=("EGO_DIRECTION=${EGO_DIRECTION}")
fi
...
if [[ -f "experiments-discrete/wandb_info/wandb_api_key" ]]; then
  export WANDB_API_KEY=$(cat experiments-discrete/wandb_info/wandb_api_key)
  echo "[INFO] WandB API key loaded from wandb_info/wandb_api_key"
  wandb login "$WANDB_API_KEY"
else
  echo "[WARN] WandB API key file not found at experiments-discrete/wandb_info/wandb_api_key"
fi
...
# 3) 실험 실행
cd experiments-discrete

export PYTHONPATH="/home/mlic/mingukang/ex-overcookedv2/experiments-discrete:${PYTHONPATH:-}"
